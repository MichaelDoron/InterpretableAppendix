...
for sample in range(number_of_samples):
	...
	frame_3 = torchvision.transforms.functional.to_tensor(torchvision.transforms.functional.to_grayscale(Image.open(data.iloc[line + 2]['frame_img']).resize((60, 80))))
	sample_images = torch.cat((frame_1, frame_2), 0)
	sample_images = torch.cat((sample_images, frame_3), 0)
	sample_images = sample_images[None, :, :, :]
	X = torch.cat((X, sample_images), 0)
	Y.append((data.iloc[line + 9]['X'], data.iloc[line + 9]['Y']))
